...
export function validateKnowledgeSnapshot(snapshot: KnowledgeSnapshot): string[] {
  const errors: string[] = [];
  ...
  if (!Array.isArray(snapshot?.records) || snapshot.records.length === 0) errors.push('records must be non-empty');
...
export class KnowledgeStore {
  private readonly byId = new Map<string, KnowledgeRecord>();
  ...
  constructor(snapshot: KnowledgeSnapshot) {
    ...
    if (errors.length) throw new Error(`Invalid knowledge snapshot: ${errors.join('; ')}`);
